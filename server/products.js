...
let findAll = (req, res, next) => {
    ...
    let sql = ["SELECT f.name as frb_name, ",
               ...
               "rmp.snr as rmp_snr, ",
               "regexp_replace(rmp.width::text,'-1','') as rmp_width, ",
               ...
               "WHERE (rmp.rank = 1 AND f.private = FALSE AND o.detected = TRUE) ORDER BY f.name,o.utc"].join('\n');
...
let findByFRB = (req, res, next) => {
  ...
  "COALESCE(regexp_replace(rmp.width::text,'-1','') || '<span className=''supsub''><sup>+' || rmp.width_error_upper::text || '</sup><sub>-'|| rmp.width_error_lower::text || '</sub></span>', rmp.width::text) as rmp_width_frmt, ",
